...
export interface IMCPService {
	...
	/** Human-readable path, e.g. ~/.void-editor-dev/mcp.json when running from sources */
	getMcpConfigPathLabel(): string;
	getMcpServerEnv(serverName: string): Record<string, string> | undefined;
	/** Load env from cache, re-read mcp.json, or main-process connected server entry */
	ensureMcpServerEnv(serverName: string): Promise<Record<string, string> | undefined>;
	/** Main-process fs probe of home + workspace mcp.json + connected MCP client env */
	probeAtlassianEnv(workspaceMcpPaths: string[]): Promise<{ env: Record<string, string>; sources: { label: string; ok: boolean; keys: string[]; error?: string }[] } | null>;
...
class MCPService extends Disposable implements IMCPService {
	...
	private _cachedMcpConfig: MCPConfigFileJSON | null = null;
	...
	private async _addMCPConfigFileWatcher(): Promise<void> {
		const uris = await this._getMcpConfigCandidateUris();
		for (const mcpConfigUri of uris) {
	...
	public getMcpConfigPathLabel(): string {
		const appName = this.productService.dataFolderName;
		return `~/${appName}/${MCP_CONFIG_FILE_NAME}`;
	}

	public getMcpServerEnv(serverName: string): Record<string, string> | undefined {
		return this._cachedMcpConfig?.mcpServers?.[serverName]?.env;
	}

	public async ensureMcpServerEnv(serverName: string): Promise<Record<string, string> | undefined> {
		let env: Record<string, string> = {};
		const json = await this._parseMCPConfigFile();
		if (json) {
			this._cachedMcpConfig = json;
			const fromJson = json.mcpServers?.[serverName]?.env;
			if (fromJson) {
				env = { ...env, ...fromJson };
			}
		}
		if (serverName === 'atlassian') {
			const fromHome = await this._readHomeAtlassianEnvFromDisk();
			env = { ...env, ...fromHome };
		}
		try {
			const fromMain = await this.channel.call('getMcpServerEnv', { serverName }) as Record<string, string> | null | undefined;
			if (fromMain && typeof fromMain === 'object') {
				env = { ...env, ...fromMain };
			}
		} catch {
			// main process may not have connected yet
		}
		return Object.keys(env).length > 0 ? env : undefined;
	}

	public async probeAtlassianEnv(workspaceMcpPaths: string[]): Promise<{
		env: Record<string, string>;
		sources: { label: string; ok: boolean; keys: string[]; error?: string }[];
	} | null> {
		try {
			const result = await this.channel.call('probeAtlassianEnv', { workspaceMcpPaths }) as {
				env: Record<string, string>;
				sources: { label: string; ok: boolean; keys: string[]; error?: string }[];
			} | null;
			return result ?? null;
	...
	/** Merge ~/.void-editor-dev and ~/.void-editor atlassian env (later files override). */
	private async _readHomeAtlassianEnvFromDisk(): Promise<Record<string, string>> {
		const userHome = await this.pathService.userHome();
		const merged: Record<string, string> = {};
		for (const sub of ['.void-editor-dev', '.void-editor']) {
			try {
				const uri = URI.joinPath(userHome, sub, MCP_CONFIG_FILE_NAME);
	...
	/** Dev builds often use ~/.void-editor-dev; product.dataFolderName may still be .void-editor */
	private async _getMcpConfigCandidateUris(): Promise<URI[]> {
		const userHome = await this.pathService.userHome();
		const appName = this.productService.dataFolderName;
		const uris: URI[] = [];
		if (appName === '.void-editor') {
			uris.push(URI.joinPath(userHome, '.void-editor-dev', MCP_CONFIG_FILE_NAME));
		}
		uris.push(URI.joinPath(userHome, appName, MCP_CONFIG_FILE_NAME));
		return uris;
	}
	...
	private async _getMCPConfigFilePath(): Promise<URI> {
		...
		for (const uri of uris) {
			...
			try {
				...
				if (configFileJson.mcpServers?.atlassian) {
					return uri;
				}
			} catch {
				// try next
	...
	private async _parseMCPConfigFile(): Promise<MCPConfigFileJSON | null> {
		const uris = await this._getMcpConfigCandidateUris();
		const merged: MCPConfigFileJSON = { mcpServers: {} };
		let readAny = false;
		...
		for (const mcpConfigUri of uris) {
			try {
				...
				readAny = true;
				for (const serverName of Object.keys(configFileJson.mcpServers)) {
					const entry = configFileJson.mcpServers[serverName];
					const prev = merged.mcpServers[serverName];
					merged.mcpServers[serverName] = prev
						? {
							...prev,
							...entry,
							env: { ...prev.env, ...entry.env },
						}
						: { ...entry };
				}
			} catch (error) {
		...
		if (readAny && Object.keys(merged.mcpServers).length > 0) {
			return merged;
		}
		const fullError = `Error parsing MCP config file: ${lastError}`;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		this._cachedMcpConfig = newConfigFileJSON;
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		const serverEntry = mcpConfigFileJSON?.mcpServers?.[serverName];
		this.channel.call('toggleMCPServer', { serverName, isOn, serverEntry })
	}
