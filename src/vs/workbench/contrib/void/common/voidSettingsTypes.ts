...
export const subTextMdOfProviderName = (providerName: ProviderName): string => {
	...
	if (providerName === 'googleVertex') return 'You must authenticate before using Vertex with Agentic_MPS. Read more about endpoints [here](https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/call-vertex-using-openai-library), and regions [here](https://cloud.google.com/vertex-ai/docs/general/locations#available-regions).'
...
/** Default auto-approve for built-in tools (edits + terminal). MCP tools stay manual for safety. */
export const defaultProactiveAutoApprove: GlobalSettings['autoApprove'] = {
	edits: true,
	terminal: true,
	'MCP tools': false,
}

export const defaultProactiveAiInstructions = `You are working in the MPS editor (Cursor-style agent fork). Be conversational, proactive, and execution-focused:

- Talk naturally with the user—brief status updates while you work ("Opening the file…", "Applying the fix…") are welcome.
- When the user asks for a feature, fix, or improvement, implement it end-to-end using tools (edit files, run commands, verify). Do not stop at a plan unless they explicitly ask for one only.
- Prefer doing over asking: if the request is clear, start implementing immediately. Only ask clarifying questions when truly blocked.
- Use edit_file so changes appear live in the editor; the user reviews green/red highlights and accepts or rejects.
- After changes, run relevant checks (build, tests, lint) when reasonable and fix obvious failures before reporting done.
- Keep going until the task is complete or you hit a real blocker—do not end with "I can do X if you want."`
...
export type GlobalSettings = {
	...
	/** Apply agent edit_file tool output into the editor while the tool is still streaming. */
	enableLiveAgentEdits: boolean;
}

export const defaultGlobalSettings: GlobalSettings = {
	autoRefreshModels: true,
	aiInstructions: defaultProactiveAiInstructions,
	enableAutocomplete: false,
	syncApplyToChat: true,
	syncSCMToChat: true,
	enableFastApply: true,
	chatMode: 'agent',
	autoApprove: { ...defaultProactiveAutoApprove },
	showInlineSuggestions: true,
	includeToolLintErrors: true,
	isOnboardingComplete: false,
	disableSystemMessage: false,
	autoAcceptLLMChanges: false,
	enableLiveAgentEdits: true,
}
