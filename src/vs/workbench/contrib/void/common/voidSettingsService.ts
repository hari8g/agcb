...
class VoidSettingsService extends Disposable implements IVoidSettingsService {
	...
	async readAndInitializeState() {
		...
		try {
			...
			// add autoAcceptLLMChanges feature — default off so users review green highlights before accepting
			if (readS.globalSettings.autoAcceptLLMChanges === undefined) readS.globalSettings.autoAcceptLLMChanges = false;
			if (readS.globalSettings.enableLiveAgentEdits === undefined) readS.globalSettings.enableLiveAgentEdits = true;

			// proactive defaults: auto-approve edits/terminal if user never configured them
			const aa = readS.globalSettings.autoApprove
			if (aa.edits === undefined) aa.edits = true
			if (aa.terminal === undefined) aa.terminal = true
			if (aa['MCP tools'] === undefined) aa['MCP tools'] = false
		}
		catch (e) {
			readS = defaultState()
		}
