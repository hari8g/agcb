...
export type InternalToolInfo = {
	...
	/** Raw MCP inputSchema from the server (used for JIRA param building). */
...
${mode === 'agent' ? `to help the user develop, run, and ship changes in their codebase. You implement solutions, not just describe them.`
...
${directoryStr}
	...
	if (mode === 'agent') {
		...
		details.push('Be conversational: give short, natural progress updates while you work (one sentence is enough). Avoid robotic tool announcements.')
		details.push('Be proactive: when the user requests a feature, fix, or refactor, implement it fully. Do not end with only suggestions or ask "should I proceed?" if the intent is already clear.')
		details.push('Prioritize taking as many steps as you need to complete the user\'s goal over stopping early. Continue until the task is done or you are blocked on missing information.')
		details.push('File edits apply live in the editor (green = additions, red = deletions). The user accepts or rejects when you are done—do not assume changes are final until they approve unless auto-accept is enabled.')
		details.push('After implementing, verify when practical: run build/test/lint commands and fix straightforward failures before you report completion.')
		details.push(`You will OFTEN need to gather context before making a change. Do not edit blindly—read, search, and inspect until you understand the affected code.`)
		details.push(`ALWAYS have maximal certainty in a change BEFORE you make it. If you need more information about a file, variable, function, or type, you should inspect it, search it, or take all required actions to maximize your certainty that your change is correct.`)
		details.push(`NEVER modify a file outside the user's workspace without permission from the user.`)
		details.push('Briefly summarize what you changed when you finish; keep the summary short and factual.')
	}
...
export const messageOfSelection = async (
	...
	opts: {
		...
		codebaseIndexService?: { search(query: string, maxResults?: number): Promise<{ uri: URI; line: number; snippet: string }[]> },
		folderOpts: {
			maxChildren: number,
	...
	if (s.type === 'CodeSelection') {
		...
		const results = await opts.codebaseIndexService?.search(s.query, 20) ?? [];
		if (results.length === 0) {
			return `@codebase (${s.query}): no matches in indexed workspace.`;
		}
		const blocks = results.map((r, i) => {
			const path = r.uri.fsPath;
			return `### Match ${i + 1}: ${path}:${r.line}\n${tripleTick[0]}\n${r.snippet}\n${tripleTick[1]}`;
		}).join('\n\n');
		return `@codebase search for "${s.query}":\n\n${blocks}`;
	}
