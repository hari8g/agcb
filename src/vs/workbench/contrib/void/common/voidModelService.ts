...
class VoidModelService extends Disposable implements IVoidModelService {
	...
	initializeModel = async (uri: URI) => {
		try {
			...
			// Optional workspace rules + binary assets are normal — don't spam the console.
			if (
				msg.includes('Unable to resolve nonexistent file')
				|| msg.includes('Unable to read file')
				|| msg.includes('binary and cannot be opened as text')
			) {
				return;
			}
