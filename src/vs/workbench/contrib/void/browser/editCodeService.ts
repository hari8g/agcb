...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	private readonly _pendingChangeNotifHandles = new Map<string, INotificationHandle>();

	private readonly _agentLiveSessions = new Map<string, {
		uri: URI;
		mode: 'search_replace' | 'rewrite';
		baselineCode: string;
		diffZone: DiffZone;
		onFinishEdit: () => void;
		lastDoneBlockCount: number;
	...
	// 		message: `Agentic_MPS Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		// green decoration and minimap decoration (new/changed lines)
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'void-greenBG void-diff-insertion', {
		...
		if (diffZone.type === 'DiffZone') {
			// Accept | Reject widget (above insertions/edits, below deletions)
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					if (diff.type === 'insertion' || diff.type === 'edit') {
						startLine = Math.max(1, diff.startLine)
						placement = 'above'
					}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Agentic_MPS: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public instantlyApplySearchReplaceBlocks({ uri, searchReplaceBlocks }: { uri: URI, searchReplaceBlocks: string }) {
		...
		const onDone = () => {
			...
			this._refreshStylesAndDiffsInURI(uri)
			this._afterDiffZoneApplyFinished(uri, onFinishEdit)
		}
	...
	public hasActiveAgentLiveEdit(uri: URI): boolean {
		return this._agentLiveSessions.has(uri.fsPath)
	}

	public beginAgentLiveEdit(uri: URI, mode: 'search_replace' | 'rewrite' = 'search_replace'): void {
		if (this._agentLiveSessions.has(uri.fsPath)) return
		if (this._uriIsStreaming(uri) && !this._agentLiveSessions.has(uri.fsPath)) return

		const { model } = this._voidModelService.getModel(uri)
		if (!model) return

		const baselineCode = model.getValue(EndOfLinePreference.LF)
		const res = this._startStreamingDiffZone({
			uri,
			streamRequestIdRef: { current: null },
			startBehavior: 'keep-conflicts',
		...
		this._agentLiveSessions.set(uri.fsPath, {
			uri,
			mode,
			baselineCode,
			diffZone,
			onFinishEdit,
			lastDoneBlockCount: 0,
			debounceTimer: null,
		})

		this._editorService.openEditor({ resource: uri, options: { pinned: false, preserveFocus: false } })
		this._refreshStylesAndDiffsInURI(uri)
	}

	public updateAgentLiveEdit({ uri, searchReplaceBlocks }: { uri: URI; searchReplaceBlocks: string }): void {
		const session = this._agentLiveSessions.get(uri.fsPath)
		if (!session) return

		if (session.debounceTimer) clearTimeout(session.debounceTimer)
		session.debounceTimer = setTimeout(() => {
			session.debounceTimer = null
			this._applyAgentLiveEditPartial(session, searchReplaceBlocks)
		}, 16)
	}

	public updateAgentLiveRewrite({ uri, partialContent }: { uri: URI; partialContent: string }): void {
		const session = this._agentLiveSessions.get(uri.fsPath)
		if (!session || session.mode !== 'rewrite') return

		if (session.debounceTimer) clearTimeout(session.debounceTimer)
		session.debounceTimer = setTimeout(() => {
			session.debounceTimer = null
			try {
				this._writeURIText(uri, partialContent, 'wholeFileRange', { shouldRealignDiffAreas: true })
				this._refreshStylesAndDiffsInURI(uri)
			} catch { /* partial JSON/content */ }
		}, 16)
	}

	public finalizeAgentLiveRewrite({ uri, newContent }: { uri: URI; newContent: string }): void {
		const session = this._agentLiveSessions.get(uri.fsPath)
		if (!session) {
			this.instantlyRewriteFile({ uri, newContent })
			return
		}
		if (session.debounceTimer) clearTimeout(session.debounceTimer)
		this._agentLiveSessions.delete(uri.fsPath)
		const { diffZone, onFinishEdit } = session
		const onDone = () => {
			diffZone._streamState = { isStreaming: false }
			this._onDidChangeStreamingInDiffZone.fire({ uri, diffareaid: diffZone.diffareaid })
			this._refreshStylesAndDiffsInURI(uri)
	...
	public finalizeAgentLiveEdit({ uri, searchReplaceBlocks }: { uri: URI; searchReplaceBlocks: string }): void {
		const session = this._agentLiveSessions.get(uri.fsPath)
		if (!session) {
		...
		try {
			const newCode = this._applySearchReplaceDoneBlocksToCode(session.baselineCode, searchReplaceBlocks)
	...
	public cancelAgentLiveEdit(uri: URI): void {
		const session = this._agentLiveSessions.get(uri.fsPath)
		if (!session) return
		if (session.debounceTimer) clearTimeout(session.debounceTimer)
		this._agentLiveSessions.delete(uri.fsPath)
		session.diffZone._streamState = { isStreaming: false }
		this.acceptOrRejectAllDiffAreas({ uri, removeCtrlKs: false, behavior: 'reject', _addToHistory: false })
	}

	public cancelAllAgentLiveEdits(): void {
		for (const fsPath of [...this._agentLiveSessions.keys()]) {
			this.cancelAgentLiveEdit(URI.file(fsPath))
		}
	}

	private _applyAgentLiveEditPartial(session: {
		uri: URI;
		baselineCode: string;
		lastDoneBlockCount: number;
	}, blocksStr: string) {
		const allBlocks = extractSearchReplaceBlocks(blocksStr)
		const applicableBlocks = allBlocks.filter(b => b.state === 'done' || b.state === 'writingFinal')
		if (applicableBlocks.length <= session.lastDoneBlockCount) return

		try {
			const newCode = this._applySearchReplaceBlocksToCode(session.baselineCode, applicableBlocks)
			this._writeURIText(session.uri, newCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
			session.lastDoneBlockCount = applicableBlocks.length
			this._refreshStylesAndDiffsInURI(session.uri)
		} catch {
			// partial blocks may not match yet — wait for more stream text
		}
	}

	private _applySearchReplaceDoneBlocksToCode(baselineCode: string, blocksStr: string): string {
		const doneBlocks = extractSearchReplaceBlocks(blocksStr).filter(b => b.state === 'done')
		if (doneBlocks.length === 0) throw new Error(`No Search/Replace blocks were received!`)
		return this._applySearchReplaceBlocksToCode(baselineCode, doneBlocks)
	}

	private _applySearchReplaceBlocksToCode(modelStr: string, blocks: ExtractedSearchReplaceBlock[]): string {
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Agentic_MPS: diff.type not recognized on: ${from}`)
		}
	...
	private _instantlyApplySRBlocks(uri: URI, blocksStr: string) {
		...
		const newCode = this._applySearchReplaceDoneBlocksToCode(modelStr, blocksStr)
		this._writeURIText(uri, newCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Agentic_MPS: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	private _countPendingDiffsOnURI(uri: URI): number {
		let count = 0
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] ?? []) {
			const diffArea = this.diffAreaOfId[diffareaid]
			if (diffArea?.type === 'DiffZone') {
				count += Object.keys(diffArea._diffOfId).length
			}
		}
		return count
	}

	private _dismissPendingChangeNotification(uri: URI) {
		const handle = this._pendingChangeNotifHandles.get(uri.fsPath)
		if (handle) {
			handle.close()
			this._pendingChangeNotifHandles.delete(uri.fsPath)
		}
	}

	private _notifyPendingChangesIfAny(uri: URI) {
		const numDiffs = this._countPendingDiffsOnURI(uri)
		if (numDiffs === 0) {
			this._dismissPendingChangeNotification(uri)
			return
		}

		this._dismissPendingChangeNotification(uri)

		const basename = uri.path.split('/').pop() ?? uri.fsPath
		const handle = this._notificationService.prompt(
			Severity.Info,
			`AI changes in ${basename} (${numDiffs} change${numDiffs === 1 ? '' : 's'}). Review green highlights in the editor, then accept or reject.`,
			[
				{
					label: 'Accept all',
					run: () => { this.acceptOrRejectAllDiffAreas({ uri, removeCtrlKs: false, behavior: 'accept' }) },
				},
				{
					label: 'Reject all',
					run: () => { this.acceptOrRejectAllDiffAreas({ uri, removeCtrlKs: false, behavior: 'reject' }) },
				},
			],
			{ sticky: true },
		)
		this._pendingChangeNotifHandles.set(uri.fsPath, handle)
	}

	private _afterDiffZoneApplyFinished(uri: URI, onFinishEdit: () => void) {
		onFinishEdit()
		if (this._settingsService.state.globalSettings.autoAcceptLLMChanges) {
			...
			this._notifyPendingChangesIfAny(uri)
		}
	}
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Agentic_MPS error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		{ editor, onAccept, onReject, diffid, startLine, placement }: {
			editor: ICodeEditor;
			onAccept: () => void;
			onReject: () => void;
			diffid: string,
		...
		const offsetLines = placement === 'above' ? -1 : 1;
		buttons.style.transform = `translateY(${offsetLines * lineHeight}px)`;
		buttons.style.justifyContent = 'flex-end';
		buttons.style.width = '100%';
		buttons.style.pointerEvents = 'none';
		buttons.className = 'void-diff-accept-inline';
		...
		acceptButton.style.padding = '2px 6px';
		if (placement === 'above') {
			acceptButton.style.borderBottom = 'none';
			acceptButton.style.borderTopLeftRadius = '6px';
			acceptButton.style.borderTopRightRadius = '6px';
			acceptButton.style.borderBottomLeftRadius = '0';
			acceptButton.style.borderBottomRightRadius = '0';
		} else {
		...
		if (placement === 'above') {
			rejectButton.style.borderBottom = 'none';
			rejectButton.style.borderTopLeftRadius = '6px';
			rejectButton.style.borderTopRightRadius = '6px';
			rejectButton.style.borderBottomLeftRadius = '0';
			rejectButton.style.borderBottomRightRadius = '0';
		} else {
