...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Agentic_MPS, please download the latest version! [Agentic_MPS Editor](https://voideditor.com/download-beta)!'
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Agentic_MPS Error: There was an error checking for updates. If this persists, please get in touch or reinstall Agentic_MPS [here](https://voideditor.com/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	metricsService.capture(`Agentic_MPS Update ${metricsTag}: Checking...`, {})
	const res = await voidUpdateService.check(explicit)
	if (!res) {
		const notifController = notifyErrChecking(notifService);
		metricsService.capture(`Agentic_MPS Update ${metricsTag}: Error`, { res })
		return notifController
	}
	else {
		if (res.message) {
			const notifController = notifyUpdate(res, notifService, updateService)
			metricsService.capture(`Agentic_MPS Update ${metricsTag}: Yes`, { res })
			return notifController
		}
		else {
			metricsService.capture(`Agentic_MPS Update ${metricsTag}: No`, { res })
			return null
		}
	}
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'Agentic_MPS: Check for Updates'),
		});
	}
