...
import { AlertTriangle, File, Ban, Check, ChevronRight, Dot, FileIcon, Pencil, Undo, Undo2, X, Flag, Copy as CopyIcon, Info, CirclePlus, Ellipsis, CircleEllipsis, Folder, ALargeSmall, TypeOutline, Text, Search } from 'lucide-react';
			...
			{allSelections.map((selection, i) => {
				...
				const thisKey = selection.type === 'CodeSelection' ? selection.type + selection.language + selection.range + selection.state.wasAddedAsCurrentFile + selection.uri.fsPath
					: selection.type === 'File' ? selection.type + selection.language + selection.state.wasAddedAsCurrentFile + selection.uri.fsPath
						: selection.type === 'Folder' ? selection.type + selection.language + selection.state + selection.uri.fsPath
							: selection.type === 'Codebase' ? selection.type + selection.query
								: i
					...
					selection.type === 'File' ? File
						: selection.type === 'Folder' ? Folder
							: selection.type === 'CodeSelection' ? Text
								: selection.type === 'Codebase' ? Search
									: (undefined as never)
				)
				...
				>
					...
					<span className="truncate overflow-hidden text-ellipsis"
						data-tooltip-id='void-tooltip'
						data-tooltip-content={selection.type === 'Codebase' ? `Codebase: ${selection.query}` : getRelative(selection.uri, accessor)}
...
/** Inline activity shown directly under the user's last message while the agent works. */
const VoidLiveTurnActivity = ({
	isRunning,
	userMessageText,
	reasoningSoFar,
	displayContentSoFar,
	...
	toolCallSoFar?: { name?: string; isDone?: boolean } | null;
}) => {
	if (!isRunning) return null;

	const lines: { id: string; text: string; streaming: boolean }[] = [];

	if (userMessageText?.trim()) {
		const t = userMessageText.trim();
		const preview = t.length > 100 ? `${t.slice(0, 100)}…` : t;
		lines.push({ id: 'understand', text: `Let me understand what you need: “${preview}”`, streaming: false });
	}

	if (isRunning === 'tool') {
		const name = toolCallSoFar?.name ?? 'tool';
		const label = name.replace(/_/g, ' ');
		lines.push({
			id: 'tool',
			text: toolCallSoFar?.isDone ? `Finished running ${label}.` : `I'm running ${label} in your workspace…`,
			streaming: !toolCallSoFar?.isDone,
		});
	} else if (isRunning === 'awaiting_user') {
		lines.push({ id: 'approval', text: 'I’m waiting for you to approve this step before I continue.', streaming: false });
	} else if (isRunning === 'idle') {
		lines.push({ id: 'prep', text: 'I’m preparing the next step…', streaming: true });
	} else {
		// LLM streaming
		const hasTokens = !!(reasoningSoFar?.length || displayContentSoFar?.length);
		if (!hasTokens) {
			lines.push({ id: 'think', text: 'I’m thinking through your request…', streaming: true });
		}
		if (reasoningSoFar?.trim()) {
			lines.push({ id: 'reason', text: reasoningSoFar.trim(), streaming: true });
		}
		if (displayContentSoFar?.trim()) {
			lines.push({ id: 'draft', text: displayContentSoFar.trim(), streaming: true });
		}
	}

	return (
		<div
			className='mx-1 my-2 pl-3 border-l-2 border-void-border-2 space-y-2'
			role='status'
			aria-live='polite'
		>
			{lines.map(line => (
				<div key={line.id} className='flex gap-2 items-start text-[13px] leading-snug'>
					<span className='text-void-fg-4 shrink-0 select-none'>›</span>
					<span className={`whitespace-pre-wrap break-words ${line.streaming ? 'text-void-fg-2' : 'text-void-fg-3'}`}>
						{line.text}
					</span>
					{line.streaming && (
						<span className='inline-block w-0.5 h-3.5 shrink-0 bg-void-fg-2 animate-pulse' aria-hidden />
					)}
				</div>
			))}
		</div>
...
const AssistantMessageComponent = ({ chatMessage, isCheckpointGhost, isCommitted, messageIdx, chatIsRunning }: { chatMessage: ChatMessage & { role: 'assistant' }, isCheckpointGhost: boolean, messageIdx: number, isCommitted: boolean, chatIsRunning: IsRunningType }) => {
	...
	const isStreamingLLM = !isCommitted && chatIsRunning === 'LLM'
	// While streaming, live text is shown in VoidLiveTurnActivity below the user message
	const hideBodyWhileStreaming = isStreamingLLM
	...
	const isEmpty = !chatMessage.displayContent && !chatMessage.reasoning && !isStreamingLLM
	if (isEmpty) return null

	return <>
		{/* reasoning token — only when committed (live reasoning shows in activity feed) */}
		{hasReasoning && isCommitted &&
		...
		{/* assistant message — committed turns only; live stream uses VoidLiveTurnActivity */}
		{(chatMessage.displayContent || isStreamingLLM) && !hideBodyWhileStreaming &&
			<div className={`${isCheckpointGhost ? 'opacity-50' : ''}`}>
				{chatMessage.displayContent ? (
					<ProseWrapper>
						...
						/>
						{isStreamingLLM && (
							<span className='inline-block w-0.5 h-4 ml-0.5 align-text-bottom bg-void-fg-1 animate-pulse' aria-hidden />
						)}
					</ProseWrapper>
				) : (
					<div className='flex items-center gap-2 text-sm text-void-fg-3 py-1'>
						<IconLoading />
...
const EditToolSoFar = ({ toolCallSoFar, }: { toolCallSoFar: RawToolCallObj }) => {
	...
	const desc1 = <span className='flex items-center'>
		{uriDone ?
			<>{getBasename(toolCallSoFar.rawParams['uri'] ?? 'unknown')} <span className='text-void-fg-3 ml-1'>· applying live</span></>
			: `Generating`}
		<IconLoading />
	</span>
...
export const SidebarChat = () => {
	...
	const lastUserText = lastUserMessage && lastUserMessage.role === 'user' ? lastUserMessage.displayContent : undefined

	const liveTurnActivityHTML = isRunning ? (
		<VoidLiveTurnActivity
			key='live-turn-activity'
			isRunning={isRunning}
