	...
	constructor(
		...
		this.callTool = {
			...
			rewrite_file: async ({ uri, newContent }) => {
				await voidModelService.initializeModel(uri)
				if (this.commandBarService.getStreamState(uri) === 'streaming' && !editCodeService.hasActiveAgentLiveEdit(uri)) {
					throw new Error(`Another LLM is currently making changes to this file. Please stop streaming for now and ask the user to resume later.`)
				}
				await editCodeService.callBeforeApplyOrEdit(uri)
				if (editCodeService.hasActiveAgentLiveEdit(uri)) {
					editCodeService.finalizeAgentLiveRewrite({ uri, newContent })
				} else {
			...
			edit_file: async ({ uri, searchReplaceBlocks }) => {
				...
				await editCodeService.callBeforeApplyOrEdit(uri)
				if (editCodeService.hasActiveAgentLiveEdit(uri)) {
					editCodeService.finalizeAgentLiveEdit({ uri, searchReplaceBlocks })
				} else {
		...
		this.stringOfResult = {
			...
			run_command: (params, result) => {
				...
				if (resolveReason.type === 'timeout') {
					return `${result_}\nTerminal command ran, but was automatically killed by Agentic_MPS after ${MAX_TERMINAL_INACTIVE_TIME}s of inactivity and did not finish successfully. To try with more time, open a persistent terminal and run the command there.`
				}
