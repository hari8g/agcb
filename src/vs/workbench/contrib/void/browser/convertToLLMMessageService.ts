...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from workspace rules / AI instructions):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	// Read workspace rules files (.voidrules, .cursorrules — Cursor-compatible)
	private _readRulesFileAtWorkspaceRoot(fileName: '.voidrules' | '.cursorrules'): string {
		try {
	...
	private _getWorkspaceRulesFileContents(): string {
		const voidRules = this._readRulesFileAtWorkspaceRoot('.voidrules')
		const cursorRules = this._readRulesFileAtWorkspaceRoot('.cursorrules')
		const parts: string[] = []
		if (voidRules) parts.push(voidRules)
		if (cursorRules && cursorRules !== voidRules) parts.push(cursorRules)
		return parts.join('\n\n')
	}

	// Get combined AI instructions from settings and workspace rules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.voidSettingsService.state.globalSettings.aiInstructions;
		const workspaceRules = this._getWorkspaceRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (workspaceRules) ans.push(workspaceRules)
		return ans.join('\n\n')
	}
