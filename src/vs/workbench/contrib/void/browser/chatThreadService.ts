...
import { ICodebaseIndexService } from './codebaseIndexService.js';
...
const findStagingSelectionIndex = (currentSelections: StagingSelectionItem[] | undefined, newSelection: StagingSelectionItem): number | null => {
	...
	for (let i = 0; i < currentSelections.length; i += 1) {
		...
		if (s.type === 'Codebase' && newSelection.type === 'Codebase') {
			if (s.query === newSelection.query) return i
			continue
		}

		if (s.type === 'Codebase' || newSelection.type === 'Codebase') continue
...
export interface IChatThreadService {
	...
	addUserMessageAndStreamResponse({ userMessage, threadId }: { userMessage: string, threadId: string, _chatSelections?: StagingSelectionItem[] }): Promise<void>;

	/** Creates a new thread without switching the current thread (for parallel subagents). */
	createSubagentThread(): string;
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	constructor(
		...
		@ICodebaseIndexService private readonly _codebaseIndexService: ICodebaseIndexService,
	) {
		super()
	...
	private _handleLiveAgentToolStream(toolCall: RawToolCallObj | null | undefined): void {
		if (!this._settingsService.state.globalSettings.enableLiveAgentEdits) return
		if (!toolCall) return

		if (toolCall.name === 'edit_file') {
			const uriStr = toolCall.rawParams.uri
			if (!uriStr || typeof uriStr !== 'string') return
			const uri = URI.file(uriStr)
			const blocks = toolCall.rawParams.search_replace_blocks
			void this._voidModelService.initializeModel(uri).then(() => {
				if (!this._editCodeService.hasActiveAgentLiveEdit(uri)) {
					if (toolCall.doneParams.includes('uri') || (typeof blocks === 'string' && blocks.length > 0)) {
						this._editCodeService.beginAgentLiveEdit(uri, 'search_replace')
					}
				}
				if (typeof blocks === 'string' && blocks.length > 0) {
					this._editCodeService.updateAgentLiveEdit({ uri, searchReplaceBlocks: blocks })
				}
			})
		} else if (toolCall.name === 'rewrite_file') {
			const uriStr = toolCall.rawParams.uri
			const newContent = toolCall.rawParams.new_content
			if (!uriStr || typeof uriStr !== 'string') return
			const uri = URI.file(uriStr)
			void this._voidModelService.initializeModel(uri).then(() => {
				if (!this._editCodeService.hasActiveAgentLiveEdit(uri)) {
					if (toolCall.doneParams.includes('uri') || (typeof newContent === 'string' && newContent.length > 0)) {
						this._editCodeService.beginAgentLiveEdit(uri, 'rewrite')
					}
				}
				if (typeof newContent === 'string') {
					this._editCodeService.updateAgentLiveRewrite({ uri, partialContent: newContent })
				}
			})
		}
	}
	...
	async abortRunning(threadId: string) {
		...
		this._editCodeService.cancelAllAgentLiveEdits()
			...
			while (shouldRetryLLM) {
				...
				const llmCancelToken = this._llmMessageService.sendLLMMessage({
					...
					onText: ({ fullText, fullReasoning, toolCall }) => {
						this._handleLiveAgentToolStream(toolCall)
...
*/
	...
	private async _addUserMessageAndStreamResponse({ userMessage, _chatSelections, threadId }: { userMessage: string, _chatSelections?: StagingSelectionItem[], threadId: string }) {
		...
		const userMessageContent = await chat_userMessageContent(instructions, currSelns, { directoryStrService: this._directoryStringService, fileService: this._fileService, codebaseIndexService: this._codebaseIndexService }) // user message + names of files (NOT content)
