...
export interface IEditCodeService {
	...
	/** Stream agent edit_file tool output into the editor before the tool finishes. */
	beginAgentLiveEdit(uri: URI, mode?: 'search_replace' | 'rewrite'): void;
	updateAgentLiveEdit(opts: { uri: URI; searchReplaceBlocks: string }): void;
	updateAgentLiveRewrite(opts: { uri: URI; partialContent: string }): void;
	finalizeAgentLiveEdit(opts: { uri: URI; searchReplaceBlocks: string }): void;
	finalizeAgentLiveRewrite(opts: { uri: URI; newContent: string }): void;
	cancelAgentLiveEdit(uri: URI): void;
	cancelAllAgentLiveEdits(): void;
	hasActiveAgentLiveEdit(uri: URI): boolean;
