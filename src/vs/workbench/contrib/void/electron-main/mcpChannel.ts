...
import { probeAtlassianEnvOnDisk, readAtlassianEnvFromHomeMcpFiles } from './atlassianMcpEnv.js';
...
export class MCPChannel implements IServerChannel {
	...
	async call(_: unknown, command: string, params: any): Promise<any> {
		try {
			if (command === 'refreshMCPServers') {
				...
				await this._toggleMCPServer(params.serverName, params.isOn, params.serverEntry)
			}
			...
			else if (command === 'getMcpServerEnv') {
				const { serverName } = params as { serverName: string };
				const fromClient = this.infoOfClientId[serverName]?.mcpServerEntryJSON?.env;
				if (serverName !== 'atlassian') {
					return fromClient ?? null;
				}
				const fromDisk = readAtlassianEnvFromHomeMcpFiles();
				if (!fromClient && !Object.keys(fromDisk).length) {
					return null;
				}
				return { ...fromDisk, ...fromClient };
			}
			else if (command === 'probeAtlassianEnv') {
				const { workspaceMcpPaths } = params as { workspaceMcpPaths?: string[] };
				const clientEnv = this.infoOfClientId['atlassian']?.mcpServerEntryJSON?.env;
				return probeAtlassianEnvOnDisk({
					workspaceMcpPaths: workspaceMcpPaths ?? [],
					clientEnv,
				});
			}
			else {
				throw new Error(`Agentic_MPS sendLLM: command "${command}" not recognized.`)
			}
		}
		catch (e) {
	...
	private _resolveServerUrl(url: string | URL): URL {
		return typeof url === 'string' ? new URL(url) : url;
	}

	private _serverUrlLabel(url: string | URL): string {
		return typeof url === 'string' ? url : url.toString();
	}

	/** HTTP/SSE auth: explicit headers, or Basic from env ATLASSIAN_EMAIL + ATLASSIAN_API_TOKEN (see Atlassian API token docs). */
	private _getHttpRequestInit(server: MCPConfigFileEntryJSON): RequestInit | undefined {
		if (server.headers) {
			return { headers: { ...server.headers } };
		}
		const env = { ...process.env, ...server.env } as Record<string, string | undefined>;
	...
	private async _createClientUnsafe(server: MCPConfigFileEntryJSON, serverName: string, isOn: boolean): Promise<ClientInfo> {
		...
		if (server.url) {
			const serverUrl = this._resolveServerUrl(server.url);
			const urlLabel = this._serverUrlLabel(server.url);
			const requestInit = this._getHttpRequestInit(server);
			const transportOpts = requestInit ? { requestInit } : undefined;
			// first try HTTP, fall back to SSE
			try {
				transport = new StreamableHTTPClientTransport(serverUrl, transportOpts);
				...
				transport = new SSEClientTransport(serverUrl, transportOpts);
				await client.connect(transport);
				const { tools } = await client.listTools()
				const toolsWithUniqueName = tools.map(({ name, ...rest }) => ({ name: this._addUniquePrefix(name), ...rest }))
	...
	private async _createClient(serverConfig: MCPConfigFileEntryJSON, serverName: string, isOn = true): Promise<ClientInfo> {
		try {
			...
			const fullCommand = serverConfig.url
				? this._serverUrlLabel(serverConfig.url)
				: (!serverConfig.command ? '' : `${serverConfig.command} ${serverConfig.args?.join(' ') || ''}`)
	...
	private async _toggleMCPServer(serverName: string, isOn: boolean, serverEntry?: MCPConfigFileEntryJSON) {
		const prevServer = this.infoOfClientId[serverName]?.mcpServer
		const existingEntry = this.infoOfClientId[serverName]?.mcpServerEntryJSON
		const entry = existingEntry ?? serverEntry

		// Handle turning on the server
		if (isOn) {
			if (!entry) {
				console.error(`Cannot enable MCP server "${serverName}": missing config (not in mcp.json or main-process cache)`)
				return
			}
			const clientInfo = await this._createClient(entry, serverName, isOn)
			this.infoOfClientId[serverName] = clientInfo
		...
		// Handle turning off the server
			...
			const info = this.infoOfClientId[serverName]
			const command = info?.mcpServer.command ?? ''
			await this._closeClient(serverName)
			if (info) {
				delete info._client
				if (info.mcpServer.status !== 'error') {
					info.mcpServer = {
						status: 'offline',
						tools: [],
						command,
