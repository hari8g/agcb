...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		actions.push(...[
			...
			toAction({ id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "Move Agentic_MPS Side Bar Left") : localize('move second side bar right', "Move Agentic_MPS Side Bar Right"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toggleShowLabelsAction,
			toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "Hide Agentic_MPS Side Bar"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}
