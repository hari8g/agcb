...
function code() {
	...
	if [[ "$OSTYPE" == "darwin"* ]]; then
		...
		CODE=""
		for candidate in \
			"./.build/electron/$NAME.app/Contents/MacOS/Electron" \
			"./.build/electron/Void.app/Contents/MacOS/Electron"; do
			if [[ -x "$candidate" ]]; then
				CODE="$candidate"
				break
			fi
		...
		if [[ -z "$CODE" ]]; then
			echo "Electron app not found. Run: rm -rf .build/electron && npm run electron" >&2
			exit 1
		fi
	else
	...
	# Chromium mishandles paths like `./.tmp` (resolves to `/.tmp`); use absolute paths.
	local cli_args=()
	while [ $# -gt 0 ]; do
		case "$1" in
			--user-data-dir|--extensions-dir)
				...
				if [[ -n "$path_arg" && "$path_arg" != /* ]]; then
					path_arg="$(cd "$ROOT" && cd "$path_arg" && pwd)"
				fi
				cli_args+=("$path_arg")
				shift
				;;
	...
	# Launch Code
	exec "$CODE" . $DISABLE_TEST_EXTENSION "${cli_args[@]}"
}
